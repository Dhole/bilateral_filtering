...
out_graph = open(out_file + '_graph.txt', 'w')
out_image = open(out_file + '_image.txt', 'w')
size_x = im.size[0]
size_y = im.size[1]
...
out_image.write('# size_x ' + str(size_x) + '\n')
out_image.write('# size_y ' + str(size_y) + '\n')

#Write the graph
for y in xrange(size_y):
	for x in xrange(size_x):
		value = pix[x,y]
		out_image.write(str(x + y * size_x) + " " + str(value) + "\n")
		...
		for j in xrange(win):
			for i in xrange(win):
				if not weights[i][j] == 0:
					x2 = x + i - mid_win
					y2 = y + j - mid_win
					line = str(x + y * size_x) + " " + str(x2 + y2 * size_x)
...
f_in = open(out_file + '_graph.txt', 'rb')
f_out = gzip.open(out_file + '_graph.txt.gz', 'wb')
f_out.writelines(f_in)
f_out.close()
f_in.close()

f_in = open(out_file + '_image.txt', 'rb')
f_out = gzip.open(out_file + '_image.txt.gz', 'wb')
f_out.writelines(f_in)
f_out.close()
f_in.close()

#Delete the uncompressed graph
os.remove(out_file + '_graph.txt')
os.remove(out_file + '_image.txt')